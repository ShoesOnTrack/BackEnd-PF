...
const createFav = async ({ UserId, ProductId }) => {
  console.log("soy el productId", ProductId);
  ...
  if (existingFavorite) {
    await existingFavorite.Products[0]?.user_products?.update({
      isFavorite: true,
    });
    return existingFavorite;
  }
  const newFavorite = await user.addProduct(ProductId, {
...
const deletefav = async ({ UserId, ProductId }) => {
  ...
  let deleted = await fav?.Products[0]?.user_products?.update({
    isFavorite: false,
  });
  return deleted;
};
